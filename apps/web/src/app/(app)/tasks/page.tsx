...
import { Task, TaskCreate, createTask, deleteTask, getTasks, updateTask } from "@/lib/tasks";
...
function toTaskPayload(form: TaskFormState): TaskCreate {
    return {
...
export default function TasksPage() {
    ...
    async function loadData() {
        ...
        try {
            const [tasksResult, projectsResult] = await Promise.all([getTasks(), getProjects()]);
            ...
            <div className="mx-auto max-w-5xl">
                <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-gray-500">Manage your IRIS tasks.</p>
                    </div>

                    <Link href="/dashboard" className="rounded border px-4 py-2 text-center">
                        Dashboard
                    </Link>
                </header>

                {error && (
                    <p className="mt-6 rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>
                )}

                <section className="mt-8">
                    <h2 className="text-xl font-semibold">{editingId ? "Edit task" : "Create task"}</h2>

                    <form onSubmit={handleSubmit} className="mt-4 grid gap-4 rounded border p-4 sm:grid-cols-2">
                                ...
                                <article key={task.id} className="rounded border p-4">
                                    ...
                                    <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-4">
                                        ...
                                        <div>
                                            ...
                                            <dd>
                                                {task.parent_task_id
                                                    ? tasks.find(t => t.id === task.parent_task_id)?.title ||
                                                      "Unknown task"
                                                    : "None"}
                                            </dd>
                                        </div>
